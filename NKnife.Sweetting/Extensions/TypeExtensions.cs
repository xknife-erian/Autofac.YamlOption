...
namespace NKnife.Sweetting.Extensions
{
    ...
    public static class TypeExtensions
    {
            ...
            if (typeof (IEnumerable).IsAssignableFrom(targetType) && (stringValue != null || typeof (IEnumerable).IsAssignableFrom(valueType)))
            {
                ...
                if (instance != null)
                {
                    ...
                    if (genericArguments.Length == 1)
                    {
                        ...
                        foreach (var item in SplitCsv(value, stringValue))
                        {
                            try
                            {
                    ...
                    if (genericArguments.Length == 2)
                    {
                        ...
                        // else
                        //     dictionary = (IDictionary) stringValue.SplitDictionary();
                        foreach (var key in dictionary.Keys)
                        {
                            try
                            {
                                addMethod.Invoke(instance, new[]
                                {
                                    valueType1.ConvertOrCastValueToType(key, convertFrom: convertFrom), 
                                    valueType2.ConvertOrCastValueToType(dictionary[key], convertFrom: convertFrom)
                                });
                            }
                            catch (Exception ex)
                            {
        ...
        private static IEnumerable SplitCsv(object value, string stringValue)
        {
            if (stringValue == null)
                return (IEnumerable) value;
            // else
            //     return stringValue.SplitCsv();
            return new string[0];
        }
